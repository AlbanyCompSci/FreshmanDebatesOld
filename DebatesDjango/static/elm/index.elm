...
debateList =
    let url = "/api/debates"
    in View {action = Http.get url, render = debateListRenderer}

teamList : View
teamList =
    let url = "/api/teams"
    in View {action = Http.get url, render = teamListRenderer}
...
type alias Debate = { time : String, judges : [User], affTeam : Team, negTeam : Team, affScores : [Score], negScores : [Score] }
mkDebate : String -> [User] -> Team -> Team -> [Score] -> [Score] -> Debate
mkDebate t js at nt ass nss = { time = t, judges = js, affTeam = at, negTeam = nt, affScores = ass, negScores = nss }

debateListRenderer : Renderer
debateListRenderer =
    let itemDecoder : Decoder Debate
        itemDecoder = object6 mkDebate
                ("time" :=
...
listHandler dec rend =
    ...
    in handler ("results" := list dec) (listOf rend)
...
type alias Listable a = { url : String, render : 
